    ...
    async def _compute_recommendation(self, run, commodity: str, ship: str) -> str:
        ...
        if committed_score is None:
            ...
            return (
                ...
                f"{best_alt.destination_terminal_name} — about "
                f"{speakable_aUEC(profit_per_hour(best_alt_score), 'aUEC/hour')}. It's {terminal_kind}."
            )

        if best_alt is None or best_alt_score <= committed_score:
            return (
                f"By profit per hour, {sale_leg.terminal_name} is still the best call — "
                f"about {speakable_aUEC(profit_per_hour(committed_score), 'aUEC/hour')}."
            )
        ...
        return (
            ...
            f"{sale_leg.terminal_name} — about {speakable_aUEC(profit_per_hour(best_alt_score), 'aUEC/hour')} "
            f"versus {speakable_aUEC(profit_per_hour(committed_score), 'aUEC/hour')}. It's {terminal_kind}."
        )
