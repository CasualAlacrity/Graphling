...
def _leg(leg_type, terminal_name, commodity_name, **overrides):
    fields = dict(
        id=uuid.uuid4(), run_id=uuid.uuid4(), leg_type=leg_type, terminal_id=1, terminal_name=terminal_name,
...
def _run(ship, legs):
    return TradeRunOut.model_validate({
...
async def test_resolve_leg_returns_the_only_match(monkeypatch):
    ...
    monkeypatch.setattr(resolver.ledger_client, "get_in_progress_runs", lambda: _async([run]))
...
async def test_resolve_leg_raises_plain_value_error_when_nothing_matches(monkeypatch):
    monkeypatch.setattr(resolver.ledger_client, "get_in_progress_runs", lambda: _async([]))
...
async def test_resolve_leg_ambiguity_message_names_ship_commodity_and_terminal(monkeypatch):
    ...
    monkeypatch.setattr(resolver.ledger_client, "get_in_progress_runs", lambda: _async([run_a, run_b]))
