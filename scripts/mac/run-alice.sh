#!/bin/bash
# Launch ALICE — the push-to-talk voice loop and the overlay, together in one process.
# There is no voice-only or overlay-only mode; this is the way it runs.
#
# Hold PTT_HOTKEY (see .env) to speak. Press OVERLAY_HOTKEY (see .env, default F3) to
# toggle the trade-run overlay.
#
...
#   - Accessibility permission: pynput's global hotkey listeners (both the PTT key and
#     the overlay toggle) need it, same place: System Settings > Privacy & Security >
#     Accessibility. Add your terminal app and enable it. If a hotkey still doesn't
#     respond after that, also check Privacy & Security > Input Monitoring — some macOS
#     versions gate key listeners there instead of (or in addition to) Accessibility.
...
if [ ! -x .venv/bin/alice ]; then
    echo "The 'alice' command is missing from the venv — this usually means the package"
    echo "was installed before it was renamed. Re-run scripts/mac/setup.sh, or:"
